...
export function getImageUrl(url) {
  ...
  // Handle local URLs returned by Django when running in production
  if (url.startsWith('http://127.0.0.1:8000') || url.startsWith('http://localhost:8000')) {
    if (!local) {
      return url.replace(/^http:\/\/(127\.0\.0\.1|localhost):8000/, 'https://api.akabirprokashoni.com');
    }
    return url;
  }
  ...
  const base = local ? 'http://127.0.0.1:8000' : 'https://api.akabirprokashoni.com';
